...
$query="SELECT username, gruppo, titolo,punteggioArena FROM users  ORDER BY punteggioArena DESC LIMIT ".$rowsPerPage." OFFSET ".$records;
$result=$mysqli->query($query);

echo '<div class="table-responsive"><table id="tablerank" class="table sortable text-center tableranking">';
     echo "<thead><tr><th>#</th><th>".$rankingsTableLabel1."</th><th>".$rankingsTableLabel2."</th><th>".$rankingsTableLabel11."</th><th>".$rankingsTableLabel8."</th></tr></thead>";
		for($i=($page*$rowsPerPage)-$rowsPerPage+1;$row=$result->fetch_assoc();$i++){
       
          if(!isset($row["punteggioArena"])) $punteggioTotale="-";
          else $punteggioTotale=$row["punteggioArena"];
  ...
  <ul class="pagination">
    ...
    <?php
       ...
       for($i=$page-(($page>=$range)?$range:$page)+1; $i<=$pagine && $i <= $page+$range; $i++){
        if($i==$page)
          echo "<li class='page-item'><a class='page-link' id='page-active' href='/index/ranking-arena/".$i."'>".$i."</a></li>";
        else
          echo "<li class='page-item'><a class='page-link' href='/index/ranking-arena/".$i."'>".$i."</a></li>";
      } 
    ?>
    ...
    $pageN=$page+1;
      if($pageN<=$pagine)
        echo'<a class="page-link" href="/index/ranking-arena/'.$pageN.'" aria-label="Next">';
      else
    ...
    <li>
      ...
      if($page != $pagine && $pagine > 0)
       echo "<a class='page-link' href='/index/ranking-arena/".$pagine."'>".$pagine."</a>";
      ?>
    </li>
  </ul>
</nav>
